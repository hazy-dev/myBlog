    ...
    <div class="container">
      <div class="row">
        <div class="col-lg-8 col-md-10 mx-auto">
          	...
          	<div id="post_list_div" ></div>
	        <!-- Pager -->
	        <div class="clearfix" id="post_list_paging">
...
<script type="text/javascript">
	...
	function post_list(curPage){
	
		$.ajax({
			type: 'post',
			url: '/post_list?page=' + curPage,
			...
			success: function(result){
				...
				if( result.listCnt <= page_idx ){
					$("#post_list_paging").hide();
				}
				
				var list_html = "";
				
				for( var i= 0 ; i<result.list.length; i++ ){
					list_html += '<div class="post-preview"><a onclick="post_detail(' + "'"+ result.list[i].filename +"'" + ')"><h2 class="post-title">' + result.list[i].subject + '</h2>';
				...
				if( result.list.length == 0 ){
					list_html += '<div class="post-preview"><a><h2 class="post-title">포스트가 준비되지 않았습니다.</h2>';
					list_html += '<h3 class="post-subtitle">준비되면 업로드 하겠습니다.</h3></a>';
					list_html += '<p class="post-meta"></p></div><hr>';
				}
	
				$("#post_list_div").append(list_html);
